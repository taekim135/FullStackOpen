# Exercises 6.1 - 6.2: Unicafe Redux
#### Reducer and Store

# Exercises 6.3 - 6.8: Anecdotes Redux
#### Reducer and Store
#### Action Creators
#### Dispatching Actions
#### Uncontrolled Form

# Exercises 6.9 - 6.13: Anecdotes Redux
#### Reducer and Store @ filter
#### Refactoring the Reducer to a separate file
#### configureStore
#### createSlice for reducer and action creators


# Exercises 6.4 - 6.19: Anecdotes Redux
#### fetch API for fetching and posting data
#### async action creators with redux-thunk
#### returning a function from an action creator

# Exercises 6.20 - 6.22: Anecdotes Redux
#### React Query for fetching and posting data
#### useQuery and useMutation hooks
#### optimization with useQueryClient and invalidateQueries